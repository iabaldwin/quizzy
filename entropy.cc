...
std::vector<int> getRandomNumbers(const std::string &file_descriptor,
...
NumericType entropy(const std::string &file_descriptor) {
  ...
  for (int event_idx = 0; event_idx < num_events; event_idx++) {
    event_prob =
        static_cast<NumericType>(event_counts[event_idx] / num_samples_f);
...
int main() {
   ...
   */
  std::cout << "d = 8, entropy = " << entropy<float, 8>("/dev/random")
            << std::endl;
  std::cout << "d = 7, entropy = " << entropy<double, 7>("/dev/random")
            << std::endl;
  std::cout << "d = 64, entropy = " << entropy<float, 64>("/dev/random")
            << std::endl;
}
