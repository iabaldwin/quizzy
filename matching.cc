...
double similarity(Patch referencePatch, Patch candidatePatch);
...
// Calculate the average cosine similarity across all rows
double similarity(Patch referencePatch, Patch candidatePatch) {
  double totalSim = 0;
  for (int i = 0; i < kPatchSize; i++) {
    auto referenceRow = referencePatch[i];
    auto candidateRow = candidatePatch[i];
    double dotP = 0;
    double refMag = 0;
    double candMag = 0;
    for (int j = 0; j < kPatchSize; j++) {
      dotP += referenceRow[j] * candidateRow[j];
      refMag += referenceRow[j] * referenceRow[j];
      candMag += candidateRow[j] * candidateRow[j];
    }

    double similarity = dotP / (sqrt(refMag) * sqrt(candMag));
    totalSim += similarity;
  }
  return totalSim / kPatchSize;
