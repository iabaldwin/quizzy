...
double patchMean(const std::array<std::array<float, kPatchSize>, kPatchSize> &patch) {
    double S = 0.0;
    for (auto a: patch)
        for(auto v: a) S += v;
    return S / (kPatchSize * kPatchSize);
}

double patchSTD(const std::array<std::array<float, kPatchSize>, kPatchSize> &patch, const double mean) {
    double S = 0.0;
    double diff;
    for (auto a: patch)
        for(auto v: a) {
    ...
    return std::sqrt(S /(kPatchSize * kPatchSize - 1));
}

double similarity(const std::array<std::array<float, kPatchSize>, kPatchSize> &reference,
                  const std::array<std::array<float, kPatchSize>, kPatchSize> &candidate) {
    // zero-normalized cross-correlation measure
    // invariant to local average intensity and contrast
    // domain [-1, 1], where -1 totally uncorrelated, +1 - totally correlated

    double r_mean = patchMean(reference);
    double r_std = patchSTD(reference, r_mean);

    double c_mean = patchMean(candidate);
    double c_std = patchSTD(candidate, c_mean);
    double D = r_std * c_std;

    double measure = 0.0;

    for (int r = 0; r < kPatchSize; r++)
        for (int c = 0; c < kPatchSize; c++)
            measure += (reference[r][c] - r_mean)* (candidate[r][c] - c_mean)/D;

    return measure /(kPatchSize * kPatchSize);
}
