...
constexpr std::array<std::array<float, kPatchSize>, kPatchSize> referencePatch =
...
constexpr std::array<std::array<float, kPatchSize>, kPatchSize>
    patchCandidate1 = {
...
float euclideanDistance(
    const std::array<std::array<float, size>, size> &reference,
    const std::array<std::array<float, size>, size> &candidate) {
  float distance = 0.0f;
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      distance += std::pow(reference[i][j] - candidate[i][j], 2);
    }
  }
...
int main() {
  /*
   *Print out the similarity between the reference patch, and candidates
   */
  std::cout << euclideanDistance<kPatchSize>(referencePatch, patchCandidate1)
            << std::endl;
