...
std::pair<float, float> getLinearRegressionCoeffs(
    std::array<std::array<float, kDimensions>, kDataPoints> data_points) {
  float x_sum =
      std::accumulate(data_points.begin(), data_points.end(), 0.0f,
                      [](auto lhs, const auto rhs) { return lhs + rhs[0]; });
  float y_sum =
      std::accumulate(data_points.begin(), data_points.end(), 0.0f,
                      [](auto lhs, const auto rhs) { return lhs + rhs[1]; });
...
int main() {
  ...
  std::cout << "alpha = " << lr_coeffs.first << ", beta = " << lr_coeffs.second
            << ", mse = " << mse << std::endl;
}
